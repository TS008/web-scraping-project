#!/usr/bin/env python3
"""
Workday API Job Scraper
Specialized scraper for Workday job sites using their internal APIs
Updated with improved job_id extraction
"""
...
class WorkdayAPIJobScraper:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  ⚠️ 422 Error - Invalid request format")
    ...
    def try_different_payloads(self, api_url: str) -> Optional[Dict]:
        """Try different payload formats to find the working one."""
        ...
        for i, payload in enumerate(payloads):
            print(f"\n🧪 Trying payload format {i + 1}/{len(payloads)}:")
            
            result = self.make_api_request(api_url, payload)
            if result:
    ...
    def scrape_jobs_api(self) -> List[Dict]:
        """Scrape jobs using the Workday API."""
        print("🚀 Starting Workday API scraping...")
        
        api_url = self.build_api_url()
        print(f"🎯 API URL: {api_url}")
        
        # Try to find working payload format
        result, working_payload = self.try_different_payloads(api_url)
        
        if not result or not working_payload:
            print("❌ Could not find working API payload format")
            return []
        
        # Now paginate through all results
        all_jobs = []
        offset = 0
        limit = working_payload.get('limit', 20)
    ...
    def extract_jobs_from_api_response(self, data: Dict) -> List[Dict]:
        ...
        if isinstance(data, list):
            job_containers.append(data)
        
        for container in job_containers:
            if isinstance(container, list) and container:
                print(f"📦 Found {len(container)} jobs in container")
                
                for job_data in container:
    ...
    def save_to_csv(self, jobs: List[Dict], filename: str = None) -> str:
        ...
        if not filename:
            ...
            company = self.company_info['company']
            filename = f"{company}_jobs_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(description='Workday API job scraper')
    ...
    try:
        print("🚀 Starting Workday API job scraping...")
        ...
        if jobs:
            ...
            print("   - The API endpoint format has changed")
            print("   - Additional authentication is required")
            print("   - The site uses a different API structure")
            print("   - Rate limiting or blocking is in effect")
